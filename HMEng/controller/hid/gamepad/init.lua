...
local xbox, steam    = { "Xbox", "XInput", "Microsoft" }, { "Steam%s*Deck", "Steam%s*Controller" }
...
function Controller:get_console_from_gamepad(_gamepad)
    ...
    args.gamepad_patterns = args.gamepad_patterns or { Playstation = ps, Xbox = xbox, Nintendo = nin, SteamDeck = steam }
...
function Controller:set_HID_flags(HID_type)
    ...
    if HID_type == "keyboard" then HID.input_mode, HID.last_type = "keyboard", "keyboard"; return end
    if HID_type == "axis" then HID.controller, HID.input_mode, HID.last_type = true, "controller", "axis"
    elseif HID_type and HID_type ~= HID.last_type then
        HID.dpad,  HID.pointer     = (HID_type == "button"), _p
        HID.mouse, HID.controller  = (HID_type == "mouse"), _c or HID.controller
        HID.touch, HID.last_type   = (HID_type == "touch"), HID_type
        HID.axis_cursor            = (HID_type == "axis_cursor")
        if _c then HID.input_mode = "controller" end
        love.mouse.setVisible(self.HID.mouse)
    end
    if HID.controller or _p then return end
