...
local function _snap_focus_node(self, node)
    ...
    if self.focus_scope_allows_node and not self:focus_scope_allows_node(node) then return N end
...
function Controller:_handle_ingame_dpad_snap(button)
    ...
    if dir == "L" or dir == "R" then return self:_snap_hand_focus(dir == "L" and -1 or 1) end
    local fct, hand = self.focused and self.focused.target, self.gm and self.gm.hand
    if not (fct and hand and fct.zone == hand) then return self:_snap_hand_focus(0) end
    return Y
end
