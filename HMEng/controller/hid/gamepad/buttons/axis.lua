...
function Controller:_handle_left_thumbstick(dt, gpobj, speed, cpos, CT, CVT, norm)
    ...
    if (self:_ingame_snap_active() or self.dragging.target) and abs(l_stick_x) + abs(l_stick_y) > DEADZONE then
    ...
    local l = self.axis_buttons.l_stick;         l.current = l.previous
    
    if     abs(l_stick_x) + abs(l_stick_y) > 0.5  then  return "button", abs(l_stick_x) > abs(l_stick_y) and (l_stick_x > 0 and "dpright" or "dpleft") or (l_stick_y > 0 and "dpdown" or "dpup")
...
function Controller:_handle_right_thumbstick(dt, gpobj, speed, cpos, CT, CVT, norm)
    ...
    local r = self.axis_buttons.r_stick
    r.current = r.previous
    
    if self:_ingame_snap_active() then
        if abs(r_stick_x) + abs(r_stick_y) > 0.5 then return "button", abs(r_stick_x) > abs(r_stick_y) and (r_stick_x > 0 and "camright" or "camleft") or (r_stick_y > 0 and "camdown" or "camup")
        elseif abs(r_stick_x) + abs(r_stick_y) < 0.3 then r.current = "" end
        return
    end
...
function Controller:update_axis(dt)
    ...
    -- Right Thumbstick
    local r_kind, r_button = self:_handle_right_thumbstick(dt, gpobj, speed, cpos, CT, CVT, norm)
    if r_kind   then axis_interpretation = r_kind end
    if r_button then axis_interpretation = axis_interpretation or "button"; local r = self.axis_buttons.r_stick; r.current = r_button; end
