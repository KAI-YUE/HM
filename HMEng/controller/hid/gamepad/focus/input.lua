...
function Controller:is_focusable(focus_target)
    ...
    if self.focus_scope_allows_node and not self:focus_scope_allows_node(focus_target) then return N end
...
local function _active_options_panel(UI)
    ...
    local root = panel.attached_panel and (panel.attached_panel.widget or panel.attached_panel)
    if _find_child_by_id(root, "opt_menu_mini_page") then return panel end
end

--- Helper: shoulder/trigger step
local function _shoulder_trigger_step(b)
    if b == "leftshoulder" or b == "triggerleft" then return -1 end
    if b == "rightshoulder" or b == "triggerright" then return 1 end
end
...
function Controller:_handle_shoulder_btns(fc, input_type, b)
    ...
    local step = _shoulder_trigger_step(b);                                                if self.screen_keyboard or not step or input_type ~= "press" then return N end
    ...
    local gm = self.gm
    if OM == opt_panel then self:emit_intent("opt_tab_step", { step = step }); return Y end
    return Y
end
...
function Controller:_modal_confirm_choices()
...
function Controller:_focus_modal_confirm_choice(b, input_type)
    ...
    local yes, no = self:_modal_confirm_choices();                 if not (yes and no) then return N end
...
function Controller:_cancel_modal_confirm(b, input_type)
    ...
    local _, target = self:_modal_confirm_choices();                  if not target then return N end
    local x, y = target:put_focused_cursor()
    self.cursor_hover.target, self.hovering.target = target, target
    self:L_cursor_press(x, y)
    self.pending_modal_cancel_button = b
    return Y
end

--- Helper: release modal cancel
function Controller:_release_modal_cancel(b)
    if self.pending_modal_cancel_button ~= b then return N end
    self.pending_modal_cancel_button = nil
