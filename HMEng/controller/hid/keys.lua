...
function Controller:key_press_update(key)
    ...
    if key == "/" and not _RELEASE_MODE then self:_debug_panel(key); return end
...
--- Key press & release 
---------------------------------------------------------
function Controller:key_press(k)   self.pressed_keys[k], self.held_keys[k]     = Y, Y    end
function Controller:key_release(k) self.held_keys[k],    self.released_keys[k] = nil, Y  end
...
function Controller:key_release_update(key, dt)
    ...
    if key == "tab"                     then self:emit_intent("reset_debug") end
end
