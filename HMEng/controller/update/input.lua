...
function Controller:_handle_controller()
    ...
    if _p and not _t and _p.is(Card) and not _p.REMOVED and (not self.focus_scope_allows_node or self:focus_scope_allows_node(_p)) then if not self.coyote_fcs then self:snap_to({ node = dr.prev_target }) else self.coyote_fcs = nil end; end
    ...
    if     snap.type == "node" and not snode.REMOVED and (not self.focus_scope_allows_node or self:focus_scope_allows_node(snode)) then f.prev_target, f.target = f.target, snode; self:update_cursor()
...
local function _clear_hover_target(h) local ht = h.target; if not ht then return end; ht.states.hover.is = N; _wake_zone_layout(ht, Y); h.target = nil end
