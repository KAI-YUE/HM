...
local LANG_STEP_KEYS = { ["["] = -1, leftbracket = -1, lbracket = -1, ["]"] = 1, rightbracket = 1, rbracket = 1 }
...
function Controller:_debug_begin_new_run(key)
    ...
    gm.Fs.begin_new_run(gm, nil, { silent_start = Y })
    return Y
end

--- Helper: debug language switcher
function Controller:_debug_language_switcher(key)
    local gm = self.gm or G;     if not (gm and gm.debug_tools) then return end
    local step = LANG_STEP_KEYS[key];     if not step then return end
    self:emit_intent("debug_lang_step", { step = step })
    return Y
end
...
function Controller:_debug_panel(key)
    ...
    if PointerGrid.handle(self, key) then return end
    if self.debug_gamepad_mode and not DEBUG_PANEL_KEYS[key] then return end
    if self.SET and self.SET.pause and (key == "s" or key == "l") then return end
    ...
    if self:_debug_language_switcher(key) then return end
    ...
    if     DEBUG_PANEL_KEYS[key] then self:emit_intent("toggle_debug")
