...
function CardFront:_init_face_cache()
	...
	self.face_px_w    = self.face_px_w or 138
	self.face_px_h    = self.face_px_h or 200
	self.face_dirty   = Y
	self.face_canvas  = self.face_canvas or LG.newCanvas(self.face_px_w, self.face_px_h)
...
function CardFront:_rebuild_face_canvas()
	...
	if old_canvas then LG.setCanvas({old_canvas, stencil = true})
    else LG.setCanvas() end
