...
function Card:_draw_shadow()
    ...
    if zone and zone.config and zone.config.card_shadow_heights then sh = zone.config.card_shadow_heights end
...
function Card:_get_back_overlay()
    ...
    local bo, count            = self.back_overlay, max(#_cards, 1)
    local _o                   = ((count - _r)%20)/count
