...
--------------------------------------------------
function BattlePreviewZone:add_card(card)
...
function BattlePreviewZone:take_card(card)
...
--------------------------------------------------
function BattlePreviewZone:move(dt) Actor.move(self, dt); self:flush_card_layout() end

function BattlePreviewZone:hard_set_T(x, y, w, h)
    local T = self.T
...
function BattlePreviewZone:remove()
    for idx = #self.cards, 1, -1 do self:take_card(self.cards[idx]) end
    BattlePreviewZone.super.remove(self)
end
end
