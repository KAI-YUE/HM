...
---____________________________
--- main: floating_log_offset
---______________________________________
function M.floating_log_offset(cfg, t)
    cfg, t = cfg or {}, t or 0
    local drama, tempo  = cfg.drama or 1, cfg.tempo or 1
    t = t*tempo
    local freq, phase  = cfg.freq or 1.6, cfg.phase or 0
    local wave         = sin(freq*t + phase) + (cfg.harmonic or 0.22)*sin(1.7*freq*t + phase + 1.1)
    local settle       = exp(-(cfg.settle_damp or 4.5)*t)
    local x            = (cfg.drift or 0.006)*sin(0.45*freq*t + phase + 0.4)
    local y            = -(cfg.lift or 0.045) + (cfg.heave or 0.012)*wave + (cfg.settle_amp or 0.035)*settle*cos((cfg.settle_freq or 8)*t)
    local r            = (cfg.roll or 0.012)*cos(freq*t + phase) + (cfg.settle_roll or 0.025)*settle*sin((cfg.settle_freq or 8)*t)
    return x*drama, y*drama, r*drama
end
