...
local function _visible_scrollable_items(widget)
    local items, out = widget.scrollable_items or {}, {}
...
local function _restore_scrollable_items(widget, panel, token)
    ...
    for _, item in ipairs(widget.scrollable_items or {}) do if _child_switch_live(panel, token, item) then FadeTree.fade_tree_in(item, nil, 0); end; end
    return Y
end
...
local function _fade_child_in(child, gm, delay, panel, token)
    ...
    if _cfg.page_switch_stagger_children and child.scrollable_items then return _fade_scrollable_children_in(child, gm, delay, panel, token); end
...
function M.start(panel, widget, gm, child_cfg, delay, token)
    ...
    for _, child in ipairs(new_list) do
        ...
        elseif _cfg.page_switch_stagger_children and child.scrollable_items then  _fade_child_in(child, gm, delay, panel, token)
