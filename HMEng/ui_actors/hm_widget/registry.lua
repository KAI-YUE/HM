...
local function _copy_config_value(key, v)
    if key ~= "scroll_offset" and key:find("_offset$")  then return _edge_offset(v) end
...
--- actor config
----------------------------
--- Helper: shadow parallax copy
local function _shadow_parallax_copy(sp) if type(sp) == "table" then return { x = sp.x, y = sp.y } end; return sp end
local function _apply_actor_config(self) local cfg = self.config; if cfg.shadow_parallax then self.shadow_parallax = _shadow_parallax_copy(cfg.shadow_parallax) end end
...
--- Helper: runtime_child_widget_items
local function runtime_child_widget_items(cfg)
    local items = cfg.runtime_child_widgets;              if not items then return {} end
    if not items[1] and not (items.style or items.renderer or items.T) then return {} end
    return items[1] and items or { items }
end
...
local function child_widget_role(gm, parent, item, T)
    ...
    return { role_type = role.role_type or "Minor", major = role.major or major, offset = role.offset or { x = T.x or 0, y = T.y or 0 }, xy_bond = role.xy_bond or "Strong", wh_bond = role.wh_bond or "Strong", r_bond = role.r_bond or "Strong", scale_bond = role.scale_bond or "Strong" }
end
...
--- Helper: init_runtime_child_widgets
local function init_runtime_child_widgets(self, gm, renderer)
    if renderer and renderer.handles_child_widgets then return end
    local items = runtime_child_widget_items(self.config); if #items <= 0 then return end
