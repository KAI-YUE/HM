...
--- Helper: stroke shadow offset
local function _stroke_shadow_offset(stroke, fallback, p_dist)
    local sp = stroke.shadow_parallax or stroke.parallax; if not sp then return fallback end
    return { x = -(sp.x or 0)*p_dist, y = -(sp.y or 0)*p_dist }
end

--- Helper: draw stroke sprite
...
local function _draw_stroke_layer(self, img, sprites, wpx, hpx, offset, shadow, p_dist)
    _draw_fill(self, wpx, hpx, offset.x, offset.y, shadow)

    LG.setColor(_stroke_color(self, shadow))
    for _, sprite in ipairs(sprites) do _draw_stroke_sprite(img, sprite, wpx, hpx, shadow and _stroke_shadow_offset(sprite.config, offset, p_dist) or offset) end
end

--- Helper: draw strokes per shadow
local function _draw_strokes_per_shadow(self, img, sprites, wpx, hpx, shadow_offset, face_offset, p_dist)
    ...
    for _, sprite in ipairs(sprites) do
        LG.setColor(_stroke_color(self, true));  _draw_stroke_sprite(img, sprite, wpx, hpx, _stroke_shadow_offset(sprite.config, shadow_offset, p_dist))
...
function M.draw(self, opts)
    ...
    if cfg.stroke_shadow_order == "per_stroke" and cfg.shadow and not opts.skip_shadow and not opts.shadow_only then _draw_strokes_per_shadow(self, img, sprites, wpx, hpx, shadow_offset, face_offset, p_dist); LG.pop(); return end
    if cfg.shadow and not opts.skip_shadow then _draw_stroke_layer(self, img, sprites, wpx, hpx, shadow_offset, true, p_dist) end
    if not opts.shadow_only then _draw_stroke_layer(self, img, sprites, wpx, hpx, face_offset, false, p_dist) end
