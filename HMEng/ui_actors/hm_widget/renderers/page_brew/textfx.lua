...
--- Helper: _fake_align_text
local function _fake_align_text(text, width)
    text, width = tostring(text or ""), tonumber(width)
    if not width or width <= #text then return text end
...
local function _new_card_textfx(self, gm, item, alpha, paint_seed_entry)
    ...
    if args.text_fake_align_width then args.text = _fake_align_text(args.text, args.text_fake_align_width) end
...
--- Helper: text hint input mode
local function _text_hint_input_mode(self, cfg)
    if cfg.text_hint_show_when ~= "pointer" then return Y end
    local HID = self.Ctrl and self.Ctrl.HID
    return HID and (HID.mouse or HID.touch) or N
end
...
local function _textfx_hint_fx_mask(self, fx)
    ...
    if not _text_hint_input_mode(self, cfg)         then return _clear_textfx_hover(cfg) end
