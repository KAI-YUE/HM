...
--- page layer
--------------------------------------------------
--- Helper: draw_page_layer
local function draw_page_layer(self, tz, draw)
    push_draw_trans(self, draw.scale)
    LG.scale(1/tz)

    local shader_on, old_shader = ApplyShader.apply_fx_mask_shader(self, draw.wpx, draw.hpx)
    draw.fn()
    LG.pop()
...
function M.draw(self)
    ...
    local scale = pressed and 0.985 or 1
    draw_page_layer(self, tz, { scale = scale, wpx = wpx, hpx = hpx, fn = function()
        Regions.draw(self, wpx, hpx, fdx, fdy)
        Polygon.draw(self, wpx, hpx, fdx, fdy)
    end })

    Children.draw(self, "under_stroke")
    draw_page_layer(self, tz, { scale = scale, wpx = wpx, hpx = hpx, fn = function() Strokes.draw(self, wpx, hpx, fdx, fdy, dx - fdx, dy - fdy) end })

    Children.draw(self)
    if not card_textfx_first then _draw_textfx(self, { skip_shadow = card_textfx_shadow_first }) end
    Children.draw(self, "overlay")
end
