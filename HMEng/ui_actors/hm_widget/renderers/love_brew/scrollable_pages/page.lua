...
function M.page(self, dir, count)
    local cfg, n = self.config, #(self.scrollable_page_items or {});                 if n <= 0 or cfg.page_transition then return N end
...
function M.set_progress(self, progress)
    local cfg, n = self.config, #(self.scrollable_page_items or {})
