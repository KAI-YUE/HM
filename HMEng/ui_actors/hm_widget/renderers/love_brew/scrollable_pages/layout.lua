...
local function _layout_visible(self)
    local items, cfg = self.scrollable_page_items or {}, self.config
    local n = #items
    if n <= 0 then return end

    local sample   = items[1].scrollable_page_base or items[1].T
...
local function _draw_children(self)
    ...
    for _, child in ipairs(self.scrollable_page_items or {}) do if child.states.visible then child:draw() end; end
end
