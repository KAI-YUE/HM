...
local function _clear_ctrl_slot_targets(self)
    ...
    for _, key in ipairs({ "clicked", "hovering", "focused", "cursor_hover", "cursor_down", "cursor_up" }) do
        local state = Ctrl[key]
        if state and Slot.is_entry_item(self, state.target) then state.target, state.handled = nil, Y end
        if state and Slot.is_entry_item(self, state.prev_target) then state.prev_target = nil end
    end
end
...
local function _lock_slot_controls(self)
    local lock = {}
    self.scrollable_entry_ctrl_lock = lock

    for _, child in ipairs(self.scrollable_items or {}) do
        local st = child.states
        lock[child] = { hover_can = st.hover.can, click_can = st.click.can, focus_can = st.focus.can }
        st.hover.can, st.hover.is, st.click.can, st.focus.can, st.focus.is = N, N, N, N, N
    end
...
local function _unlock_slot_controls(self)
    local lock = self.scrollable_entry_ctrl_lock;            if not lock then return end
    for child, saved in pairs(lock) do if not child.REMOVED then local st = child.states; st.hover.can, st.click.can, st.focus.can = saved.hover_can, saved.click_can, saved.focus_can; end; end

    self.scrollable_entry_ctrl_lock = nil
end
