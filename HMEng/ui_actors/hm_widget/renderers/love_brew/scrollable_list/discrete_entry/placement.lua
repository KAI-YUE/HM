...
local function _hide_all(self)                     for _, child in ipairs(self.scrollable_items or {}) do child.disable_button, child.states.visible, child.scrollable_entry_slot = Y, N, nil end; end
...
local function _place_child_at_slot(self, child, sample, slot)
    local cfg, base  = self.config, child.scrollable_item_base or child.T
    local x,   y     = _item_base(self, sample, slot)
    local bx,  by    = _slot_bias(cfg, slot)
    local visible    = Slot.visible_count(cfg, #(self.scrollable_items or {}))
    ...
    child.scrollable_entry_slot                  = slot
    _sync_child_tree_to_slot(child, child)
    local Ctrl = self.Ctrl;     if Ctrl and Ctrl.focused and Ctrl.focused.target == child and Ctrl.update_cursor then Ctrl:update_cursor() end
end
