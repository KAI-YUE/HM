...
--- Helper: outside active modal
local function _outside_active_modal(self, gm)
    local owner = gm and gm.UI and gm.UI.modal_backdrop and gm.UI.modal_backdrop.owner; if not (owner and not owner.REMOVED) then return N end
    local root, node = owner.widget or owner, self
    while node do if node == root then return N end; node = node.parent end
    return Y
end

--- Helper: _gamepad_button_visual_active
local function _gamepad_button_visual_active(cfg, Ctrl)
    local held = Ctrl and Ctrl.held_buttons;              if not held then return N end
    if cfg.hid_button and held[cfg.hid_button] then return Y end
    local action = cfg.hid_action;                        if action == "delete" then action = "secondary" end
    local list = action and Ctrl.gamepad_button_action_list and Ctrl:gamepad_button_action_list(action); if not list then return N end
    for _, button in ipairs(list) do if held[button] then return Y end end
    return N
end
...
function HMWidget:button_visual_active()
    ...
    if self.under_overlay or _outside_active_modal(self, gm) then return N end
    if cfg.parent_press_squash and self.parent and self.parent.button_visual_active and self.parent:button_visual_active() then return Y end

    local Ctrl   = gm.CTRL
    if _gamepad_button_visual_active(cfg, Ctrl) then return Y end
    if not cfg.button then return N end
