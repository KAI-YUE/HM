...
-----------------------------
function CardTextFx:button_visual_active()
    local gm, cfg = self.gm, self.config;                 if not (gm and cfg and cfg.button) then return N end
    local Ctrl, cdown = gm.CTRL, gm.CTRL and gm.CTRL.cursor_down
    if Ctrl and Ctrl.is_cursor_down and cdown and cdown.target == self then return Y end
    return self.last_clicked and gm._T.real_s < self.last_clicked + (cfg.click_visual_time or 0.1) or N
end
