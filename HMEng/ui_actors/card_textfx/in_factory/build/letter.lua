...
function CardTextFx:_push_space(cache, font_cfg, scale)
    ...
    local space_w     = (font and font:getWidth(" ") or 8) * scale * font_scale + _letter_space
    if self.config and self.config.textfx_space_bounds == Y then cache.bounds.x0, cache.bounds.x1 = min(cache.bounds.x0 or cache.x, cache.x), max(cache.bounds.x1 or cache.x + space_w, cache.x + space_w) end
    cache.x = cache.x + space_w
    if self.config and self.config.textfx_space_bounds == Y then cache.line_w = max(cache.line_w, cache.x) end
end
