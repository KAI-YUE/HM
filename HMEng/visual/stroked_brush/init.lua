...
local _send_base    = ShaderUtils.send_base_uniforms
...
--- Helper: paint shader name
local function _paint_shader(paint)
    if type(paint) == "string" then return paint end
    return type(paint) == "table" and (paint.shader or paint[1])
end

--- Helper: apply paint shader
local function _apply_paint(self, shadow)
    if LG.getShader() then return end
    local cfg, gm = self.config, self.gm
    local paint = cfg.paint;                              if not paint then return end
    local shader_name = _paint_shader(paint)
    local shader = shader_name and gm.t_shaders[shader_name]; if not shader then return end

    local qx, qy, qw, qh = self.quad:getViewport()
    local iw, ih = self.img:getDimensions()
    local now = gm._T.real_s or 0
    paint = type(paint) == "table" and paint or {}

    _send_base(shader, { hovering = 0, hover_tilt = 0, position_shader_mode = 0,
        fx_mask = 0, time = now, tex_details = { qx, qy, qw, qh }, image_details = { iw, ih }, shadow = shadow })
    _send_uniform(shader, "speed", paint.speed or 1)
    _send_uniform(shader, shader_name, { 0, now, paint.shader_id or paint.seed or 0 })
...
local function _draw_pass(self, geom, args, pass)
    ...
    local paint_on, old_paint = _apply_paint(self, pass.shadow)
    ...
    if paint_on then LG.setShader(old_paint) end
