...
function GameObj:_to_container(p)
    local t  = self.args[Tcelems[2]];          if not t then return end 
...
function GameObj:hit_test(point)
	local clip = self.scrollable_clip_parent

    if clip and (clip.REMOVED or not clip.scrollable_contains_point or not clip:scrollable_contains_point(point)) then return N end
