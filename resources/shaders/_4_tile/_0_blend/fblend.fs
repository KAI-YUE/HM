...
extern highp number light_boost;
extern highp vec2 world_phase;

const highp number GameID = 0.0;

const highp number PerspectiveCenter = 3;  // Moves the perspective horizon; higher values flatten the projected tile field.
const highp number PerspectiveSlope = 0.5; // Controls how quickly perspective changes from bottom to top of screen.
const highp number Depth = 125;           // Scales mip-like fractal depth; higher values push sampling toward coarser repeats.
...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc)
{
    ...
    // static center and slope (doesn't really matter, the z_t controls most of the dynamics)
    // number center = PerspectiveCenter;
    // number slope = PerspectiveSlope;

	number light_boost = light_boost * mix(0.75, 1., wave(t, 1.6, phase + 0.8));
	number perspective = 1.0 / max(abs(center - uv.y * slope), 1e-4);

    number scale = exp((cos(z_t * 0.2) + 1) * 1.5);
