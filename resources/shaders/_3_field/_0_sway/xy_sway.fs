...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc)
{
	...
	number edge_fade = smoothstep(0.0, 0.05, edge_dist);
	if (shadow) {
		return apply_fx_mask(vec4(0.0, 0.0, 0.0, warped.a * base.a * edge_fade) * color, uv01);
	}

	vec4 outc = vec4(warped.rgb, warped.a * base.a * edge_fade) * color;
