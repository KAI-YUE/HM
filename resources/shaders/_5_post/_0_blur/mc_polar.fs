...
vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc)
{
    ...
    float mc_radius = blur_radius / resolution.y * (sin(time * 0.05) + 1.0);
    float polar_shift = Strength * PolarRadius;
    ...
    vec2 r_shift = GetVector(vec, i, n.x) * polar_shift;
    vec2 g_shift = GetVector(vec, j, n.x) * polar_shift;
    vec2 b_shift = GetVector(vec, k, n.x) * polar_shift;

    vec4 c = vec4(0.0);
    float r;
    vec2 rv;

    srand(tc + n.xx + time * 0.001, r);

    for (int idx = 0; idx < ITER; idx++) {
        rv.x = rand(r);
        rv.y = rand(r);
        vec2 jitter = rv * mc_radius;

        c.r += Texel(tex, tc + jitter + r_shift).r;
        c.g += Texel(tex, tc + jitter + g_shift).g;
        c.b += Texel(tex, tc + jitter + b_shift).b;
        c.a += Texel(tex, tc + jitter).a;
    }
