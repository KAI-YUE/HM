...
local function _focus_label(ctrl)
    local fct = ctrl and ctrl.focused and ctrl.focused.target;       if not fct then return "nil" end
    local cfg = fct.config or {}
    local zone = fct.zone and fct.zone.config and fct.zone.config.type
    return tostring(cfg.id or cfg.key or (fct.is and fct:is(Card) and "card") or fct.ID or "?") .. (zone and ("@" .. zone) or "")
end

-----------------------------
--- Helpers: dev hints
----------------------------
local function _dev_hints(ctrl)
    local hints = { "+: pointer mode" }
    if ctrl and ctrl.debug_gamepad_mode then hints[#hints + 1] = "*: console switch" end
    return hints
end
...
function GMgr:debug_simulated_controller()
    ...
    local msg = "simulated controller | " .. tostring(ctrl.GAMEPAD_CONSOLE or "Generic")
    if probe.button then msg = msg .. " | " .. tostring(probe.key or "?") .. " -> " .. tostring(probe.button) .. " | " .. tostring(probe.snap or "") end
    local layer = ctrl.cursor_context and ctrl.cursor_context.layer or 1
    msg = msg .. "\nscope: " .. tostring(ctrl.gamepad_focus_scope or "nil") .. " | nav_field: " .. tostring(ctrl.navigate_field == Y) .. " | layer: " .. tostring(layer)
    msg = msg .. "\nfocus: " .. _focus_label(ctrl)
...
--- fps
----------------------------
--- Helper: pointer grid overlay
function GMgr:debug_pointer_grid()
    if _RELEASE_MODE or not self.debug.on or not self.F.verbose then return end
    PointerGrid.draw_grid(self)
end
...
function GMgr:debug_fps()
    ...
    local pointer = PointerGrid.overlay_text(ctrl)
    if pointer then LG.print(pointer, 10, y0 + 30) end
    local hints, fh = _dev_hints(ctrl), LG.getFont():getHeight()
    local hy = LG.getHeight() - (#hints * (fh + 4)) - 10
    for i, hint in ipairs(hints) do
        if i == 1 then LG.setColor(ctrl and ctrl.debug_pointer_mode and 0 or 1, 1, 1, 1) end
        LG.print(hint, 10, hy + (i - 1)*(fh + 4))
        if i == 1 then LG.setColor(0, 1, 1, 1) end
    end
