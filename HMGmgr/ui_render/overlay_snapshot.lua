...
function GMgr:_send_overlay_snapshot_shader_uniforms(shader, canvas, cfg)
    ...
    if shader:hasUniform("flow_strength") then shader:send("flow_strength", cfg.underlay_flow_strength or 1.0) end
...
function GMgr:_draw_overlay_snapshot()
    ...
    local alpha       = cfg and cfg.underlay_alpha or 1
    local old_shader  = LG.getShader()
    local r, g, b, a  = LG.getColor()

    if cfg and cfg.underlay_base == "world" then self:_draw_world_field() end
    LG.push();                                  LG.origin()
    if shader then self:_send_overlay_snapshot_shader_uniforms(shader, canvas, cfg); LG.setShader(shader); end
    LG.setColor(cw[1], cw[2], cw[3], (cw[4] or 1)*alpha); LG.draw(canvas, 0, 0)
