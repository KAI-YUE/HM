...
--- init_cardzones
----------------------------------
--- Helper: instant silent hand
local function instant_silent_hand(gm, restored)
    if restored then return end
    local hand, deck = gm.hand, gm.deck;     if not (hand and deck) then return end
    local hand_space = math.min(#deck.cards, hand.config.card_limit - #hand.cards)
    for _ = 1, hand_space do
        local card = table.remove(deck.cards, 1);    if not card then break end
        card:detach_from_zone()
        hand:emplace(card)
    end
    deck:set_zone_sts()
    hand:sort()
    hand.config.enable_drag_sort, hand.config.live_layout = Y, N
    deck:align_cards()
    hand:align_cards()
end
...
function M.init_cardzones(gm, opts)
    ...
    if opts.silent_start then instant_silent_hand(gm, restored); return end
