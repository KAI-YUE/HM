...
local PROFILE, ICONS, BAR_BG = Theme.profile or {}, Theme.icons or {}, Theme.bar_bg or {}
...
local function _panel_args(gm, side)
    ...
    local children = {
        _sprite(_pass_T(panel_1_T, panel_pass),   "ui_pack", "panel_1", PANEL.pass_tint or crm, LAYER.panel - 1, "hud_panel_pass", Common.with(Common.with({}, panel_shadow), Common.pass_fit(panel_pass))),
        _sprite({ x = 0, y = 0, w = panel_T.w },  "ui_pack", "panel_1", PANEL.base_tint or cw,  LAYER.panel, "hud_panel_1", { fit_axis = "width" }),
        _sprite(_pass_T(panel_2_T, panel_2_pass), "ui_pack", "panel_2", PANEL.detail_pass_tint or PANEL.pass_tint or crm, LAYER.panel, "hud_panel_2_pass", Common.pass_fit(panel_2_pass)),
    ...
    _push_all(children, {
        _sprite(Common.copy_T(icon_T.profile[side]), "icons", PROFILE.icon and PROFILE.icon[side] or (foe and "chat" or "chef_hat"), nil, LAYER.icon, "hud_profile_icon"),
        _sprite(Common.copy_T(icon_T.hp),            "icons", ICONS.hp or "heart", nil, LAYER.icon, "hud_hp_icon"),
        _sprite(Common.copy_T(icon_T.full),          "icons", ICONS.full or "muffin", nil, LAYER.icon, "hud_full_icon"),
        _sprite(Common.copy_T(icon_T.money),         "icons", ICONS.money or "coin", nil, LAYER.icon, "hud_money_icon"),
        _sprite(Common.copy_T(bar_bg.hp),            "ui_pack", "btn_mask", BAR_BG.hp or { 1, 1, 1, 0.46 }, LAYER.bar - 1, "hud_hp_bar_bg"),
        _sprite(Common.copy_T(bar_bg.full),          "ui_pack", "btn_mask", BAR_BG.full or { 1, 1, 1, 0.46 }, LAYER.bar - 1, "hud_full_bar_bg"),
    })
