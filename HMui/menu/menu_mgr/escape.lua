...
function M.open_pause_menu(gm)
    ...
    _cfg.underlay_shader_time                                = gm._T and gm._T.real_s or 0
...
function M.handle_escape(gm)
    local gUI, ST, STS = gm.UI, gm.g_state, gm.g_states
    local OM = gUI.overlay_menu
    if gm.CTRL and gm.CTRL.locks and gm.CTRL.locks.quick_resume then return end
