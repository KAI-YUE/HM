...
local function prompt_width()
    local w = 0
    for i, tab in ipairs(prompt_letters) do w = w + (i < #prompt_letters and tab.step_w or tab.w) end
    return w
end
...
local function prompt_word_bounds(pos, first, last)
    ...
    for i = first, last do
        ...
        x0, y0 = math.min(x0 or p.x, p.x), math.min(y0 or p.y, p.y)
        x1, y1 = math.max(x1 or p.x + p.w, p.x + p.w), math.max(y1 or p.y + p.h, p.y + p.h)
        ::continue::
    end
    return x0, y0, x1, y1
end
...
local function prompt_underlay_bounds(bounds, item)
    ...
    bounds.x0, bounds.y0 = math.min(bounds.x0 or x, x), math.min(bounds.y0 or y, y)
    bounds.x1, bounds.y1 = math.max(bounds.x1 or x + w, x + w), math.max(bounds.y1 or y + h, y + h)
end
...
local function prompt_underlay_items()
    local items, bounds = {}, {}
    local paper = Layout.word_mask_paper
    ...
    for _, cfg in ipairs(word_masks) do
        local item = prompt_word_mask_item(cfg, pos)
...
local function prompt_underlay_composite_widget(out)
    ...
    out[#out + 1] = {
        style = "sprite_in_page", renderer = "sprite_composite",
        id = "press_any_underlay_composite", room_ref = Y,
        T = { x = x, y = y, w = bounds.x1 - bounds.x0 + 2*pad, h = bounds.y1 - bounds.y0 + 2*pad },
        composite_items = items, composite_blend = "lighten",
        paint = paint.underlay_sway,
        sprite_color = colors.word_mask, tint = colors.word_mask, widget_dist = 0.53,
        button = N, can_click = N, can_hover = N,
    }
end
...
local function prompt_underline_widgets(out)
    for _, cfg in ipairs(Layout.underline) do prompt_underline_widget(out, cfg) end
end
