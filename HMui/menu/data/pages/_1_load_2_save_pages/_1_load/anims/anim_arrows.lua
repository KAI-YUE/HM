...
local function _restore_arrow_enter(widget, apply)
    local normal = widget and widget._load_menu_arrow_drop
    if not normal then return Y end
    if apply ~= false then widget.draw_alpha, widget.draw_rotate = normal.draw_alpha, normal.draw_rotate end
    if widget.config then widget.config.pendulum_string = apply ~= false and normal.pendulum_string or nil end
...
local function _drop_spring(gm, widget, preset, delay)
    ...
    _after(gm, at + 0.04, function()
        if _arrow_live(widget) then _restore_draw_offset(widget, "_load_menu_arrow_drop_offset"); return _restore_arrow_enter(widget, Y) end
        _clear_draw_offset_cache(widget, "_load_menu_arrow_drop_offset")
        return _restore_arrow_enter(widget, false)
    end)
...
local function _drop_in_arrow_part(gm, widget, preset, delay)
    ...
    if preset.fade_only then
        _after(gm, start + _dt(preset, preset.fade_time or _arrow_fade_time) + 0.04, function() return _restore_arrow_enter(widget, _arrow_live(widget)) end)
        return
    end
