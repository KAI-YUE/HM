...
local function _spring_mini_page_T(gm, widget) _spring_draw_offset(gm, widget, "_load_menu_mini_page_enter", _mini_page_from, _dilated_spring(_mini_page_spring, _mini_page_dilation), _mini_at(_mini_start), _queue) end
...
function M.fade_in(gm, mini)
    ...
    for i, child in ipairs(mini.page_child_widgets or {}) do local at = _mini_at(_mini_item_start + (i - 1)*_mini_item_stagger); _spring_draw_tree(gm, child, "_load_menu_mini_child_enter", _mini_item_from, _dilated_spring(_mini_item_spring, _mini_item_dilation), at); _fade_hint_children(gm, child, at) end
    for i, fx   in ipairs(mini.page_card_textfx or {})    do _spring_draw_offset(gm, fx, "_load_menu_mini_textfx_enter", _mini_item_from, _dilated_spring(_mini_item_spring, _mini_item_dilation), _mini_at(_mini_item_start + (#(mini.page_child_widgets or {}) + i - 1)*_mini_item_stagger), _queue) end
end
