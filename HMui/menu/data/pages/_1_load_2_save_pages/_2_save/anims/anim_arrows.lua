...
--- Helper: _normal_draw_alpha | _arrow_live | _clear_draw_offset_cache
local function _normal_draw_alpha(widget) return widget and (widget.page_switch_draw_alpha or widget.draw_alpha) end
local function _arrow_live(widget) return widget and not widget.REMOVED and not widget.page_switch_fading_out end
local function _clear_draw_offset_cache(widget, key) if widget then widget[key], widget[key .. "_cached"] = nil, nil end; return Y end
...
local function _restore_arrow_enter(arrow, apply)
    local normal = arrow and arrow._save_menu_arrow_enter
    if not normal then return Y end
    if apply ~= false then arrow.draw_alpha = normal.draw_alpha end
    if arrow.config then arrow.config.pendulum_string = apply ~= false and normal.pendulum_string or nil end
...
local function _set_pendulum_frame(widget, normal, pendulum, t)
    if not _arrow_live(widget) then return Y end
...
local function _pendulum_draw_offset(gm, widget, key, pendulum, delay)
    ...
    _after(gm, (delay or 0) + duration + 0.04, function()
        if _arrow_live(widget) then if widget.config then widget.config.pendulum_string = nil end; return _restore_draw_offset(widget, key) end
        if widget.config then widget.config.pendulum_string = nil end
        return _clear_draw_offset_cache(widget, key)
    end)
...
local function _fade_in_arrow_part(gm, widget, preset, delay)
    ...
    if preset.fade_only then
        ...
        _after(gm, motion_delay + (preset.fade_time or _arrow_fade_time)*_arrow_fade_dilation + 0.04, function() return _restore_arrow_enter(widget, _arrow_live(widget)) end)
        return
    end
    ...
    _after(gm, pendulum_done, function() return _restore_arrow_enter(widget, _arrow_live(widget)) end)
end
