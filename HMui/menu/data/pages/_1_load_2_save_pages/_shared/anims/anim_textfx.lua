...
local function _cache_textfx_enter(fx)
    ...
    fx._save_menu_normal_draw_alpha = _normal_draw_alpha(fx)
...
local function _fade_in_textfx(gm, fx, delay)
    ...
    for _, child in ipairs(fx.children or {}) do if child.config and child.config.renderer == "hint_btn" then ChildFadeTree.set_tree_alpha(child, 0) end end
    _after(gm, delay, function()
        if fx.REMOVED then return Y end
        for _, child in ipairs(fx.children or {}) do if child.config and child.config.renderer == "hint_btn" then ChildFadeTree.fade_tree_in(child, gm, Slots.fade_time) end end
