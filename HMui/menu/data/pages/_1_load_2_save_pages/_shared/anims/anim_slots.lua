...
local Settings  = require("HMui.menu.data.pages._1_load_2_save_pages._shared.anims.settings")
...
local _slot_fade_time          = Settings.SLOT.fade_time
local _slot_stagger            = Settings.SLOT.stagger
local _slot_start              = Settings.SLOT.start
...
--- Helper: _uses_parent_cut_in
local function _uses_parent_cut_in(child)
    local node = child
    while node do if node.config and node.config.parent_cut_in_sync then return Y end; node = node.parent end
end
...
local function _set_slot_enter_draw_alpha(slot)
    ...
    for _, child in ipairs(_slot_child_widgets(slot)) do
        ...
        child.draw_alpha = 0
        if (cfg and cfg.slot_enter_shader and cfg.paint) or _uses_parent_cut_in(child) then child.fx_mask, child.fx_mask_dir = 1, 1 end
    end
end
...
local function _fade_slot_child_in(gm, slot, child, fade_time)
    ...
    if (child.config and child.config.slot_enter_shader and child.config.paint) or _uses_parent_cut_in(child) then return _ease(gm, child, "fx_mask", 0, fade_time, "lerp") end
    return Y
end
...
local function _visible_slot_items(slot_list)
    local items, cfg, out = slot_list and slot_list.scrollable_items or {}, slot_list and slot_list.config or {}, {}
