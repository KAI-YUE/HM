...
--------------------------------------------------
--- Helper: localized delete label
--------------------------------------------------
local function delete_label(gm)
    local text = I18N.i18n(gm, { type = "menu", key = "items.delete" })
    return text ~= "ERROR" and text or "Delete"
end
...
local function reset_visible_slot_fx_masks(list) for _, slot in ipairs((list and list.scrollable_page_items) or {}) do if slot.states and slot.states.visible then reset_slot_fx_mask(slot) end end end
...
function M.child_widgets(gm, opts)
    ...
    return {
        {   --- basics
            style      = "scrollable_pages",       id              = list_id,
        ...
        {
            style = "hid_hint",               id = page_id .. "_delete_hint",
            hid_action = "delete",            label = delete_label(gm),
            T = { x = 15.4, y = 9.7, w = 3.2, h = 0.6 },
        },
    }
end
