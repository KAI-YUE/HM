...
function M.animate_hand_fan_out(gm)
    ...
    local function hand_delay(at) return max(0, at - timeline.field_spawn) end
    ...
    local max_draw_groups = ceil(hand_cfg.card_limit/2)
    local draw_finish_delay = draw_start_delay + (max_draw_groups - 1)*(draw_group_gap + 0.03) + 0.05
    local drag_sort_delay = max(hand_delay(timeline.drag_sort), draw_finish_delay)
    ...
    enqueue_after(timeline.field_spawn, function ()
        ...
        enqueue_after(drag_sort_delay,                     function() hand_cfg.enable_drag_sort, hand_cfg.live_layout = Y, N; hand:align_cards(); return Y end)
        return Y
    end)
