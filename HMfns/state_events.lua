...
local gm = G
G.Fs.draw_from_play_to_discard = function(e) G.Fs.draw_play2discard(G, e) end 

G.Fs.discard_cards_from_highlighted = function(e, hook)
    stop_use()
    G.CTRL.interrupt.focus = true
    G.CTRL:save_cardarea_focus(G, 'hand')

    for k, v in ipairs(G.run_card_id) do
        v.ability.forced_selection = nil
    end

    if G.CTRL.focused.target and G.CTRL.focused.target.zone == G.hand then G.card_area_focus_reset = {zone = G.hand, rank = G.CTRL.focused.target.rank} end
    ...
    if highlighted_count > 0 then 
        ...
        for i=1, highlighted_count do
            ...
            if removed then
                ...
                if G.hand.highlighted[i].ability.name == 'Glass Card' then 
                    ...
                    G.hand.highlighted[i]:start_fx_mask()
                end
            else 
                G.hand.highlighted[i].ability.discarded = true
                G.Fs.draw_from_to(G, G.hand, G.discard, i*100/highlighted_count, 'up', false, G.hand.highlighted[i])
            end
        end
        ...
        if not hook then
            ...
            G.g_state = G.g_states.draw_hand
            G.E_MANAGER:enqueue_event({
                trigger = 'immediate',
                func = function()
                    G.state_comp = false
                    return true
...
G.Fs.play_cards_from_highlighted = function(e)
    ...
    G.CTRL.interrupt.focus = true
    G.CTRL:save_cardarea_focus('hand')
    ...
    G.E_MANAGER:enqueue_event({
        trigger = 'immediate',
        func = function()
            G.g_state = G.g_states.hand_played
            G.state_comp = true
            return true
        end
    })
    ...
    delay(0.4)
        ...
        for i=1, #G.hand.highlighted do
            ...
            G.Fs.draw_from_to(G, G.hand, G.play, i*100/#G.hand.highlighted, 'up', nil, G.hand.highlighted[i])
        end
        ...
        if G.GAME.blind:press_play() then
            G.E_MANAGER:enqueue_event({
                trigger = 'immediate',
                func = (function()
                    G.HUD_blind:get_UI_by_ID('HUD_blind_debuff_1'):jitter_me(0.3, 0)
                    G.HUD_blind:get_UI_by_ID('HUD_blind_debuff_2'):jitter_me(0.3, 0)
                    G.GAME.blind:jitter_me()
                    G.E_MANAGER:enqueue_event({trigger = 'after', delay = 0.06*G.SET.g_speed, blockable = false, blocking = false, func = function()
            ...
            func = (function()
                ...
                G.E_MANAGER:enqueue_event({
                    ...
                    func = function()
                        ...
                        G.Fs.draw_from_play_to_discard()
...
G.Fs.get_poker_hand_info = function(_cards) return G.Fs.poker_hand_info(G, _cards) end 
G.Fs.draw_from_play_to_hand = function(cards) G.Fs.draw_play2hand(G, cards) end 
G.Fs.draw_from_discard_to_deck = function(e)  G.Fs.draw_discard2deck(G, e) end 
G.Fs.draw_from_hand_to_deck = function(e) G.Fs.draw_hand2deck(G, e) end 
G.Fs.draw_from_hand_to_discard = function(e) G.Fs.draw_hand2discard(G, e) end 

G.Fs.evaluate_play = function(e)
    local text,disp_text,poker_hands,scoring_hand,non_loc_disp_text = G.Fs.get_poker_hand_info(G.play.cards)
    ...
    update_hand_text({sound = G.GAME.current_round.current_hand.handname ~= disp_text and 'button' , volume = 0.4, immediate = true, nopulse = nil,
    ...
    if not G.GAME.blind:debuff_hand(G.play.cards, poker_hands, text) then
        ...
        mult, hand_chips = mult, mod_chips(hand_chips)
        ...
        for i=1, #scoring_hand do
            ...
            if scoring_hand[i].debuff then
                ...
                G.E_MANAGER:enqueue_event({
                    trigger = 'immediate',
                    func = (function() G.HUD_blind:get_UI_by_ID('HUD_blind_debuff_1'):jitter_me(0.3, 0)
                        G.HUD_blind:get_UI_by_ID('HUD_blind_debuff_2'):jitter_me(0.3, 0)
                        G.GAME.blind:jitter_me();return true end)
                })
                ...
                for j=1,#reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        if effects[ii].extra then 
                            ...
                            if effects[ii].extra.mult_mod then mult = mult + effects[ii].extra.mult_mod; extras.mult = true end
                        ...
                        if effects[ii].edition then
                            ...
                            mult = mult*(effects[ii].edition.x_mult_mod or 1)
                            update_hand_text({delay = 0}, {
                                chips = effects[ii].edition.chip_mod and hand_chips ,
                                mult = (effects[ii].edition.mult_mod or effects[ii].edition.x_mult_mod) and mult ,
                            })
                ...
                while j <= #reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        if effects[ii].card then
                            ...
                            G.E_MANAGER:enqueue_event({
                                trigger = 'immediate',
                                func = (function() effects[ii].card:jitter_me(0.7);return true end)
                            })
                        end
        ...
        --+++++++++++++++++++++++++++++++++++++++++++++++++++++++++--
        percent = percent + percent_delta
        for i=1, #G.jokers.cards + #G.consumables.cards do
            local _card = G.jokers.cards[i] or G.consumables.cards[i - #G.jokers.cards]
            ...
            if edition_effects.jokers then
                ...
                if edition_effects.jokers.mult_mod then
                    mult = mult + edition_effects.jokers.mult_mod
                    update_hand_text({delay = 0}, {mult = mult})
                    card_eval_status_text(_card, 'jokers', nil, percent, nil, {
            ...
            if effects.jokers then 
                local extras = {mult = false, hand_chips = false}
                if effects.jokers.mult_mod then mult = mult + effects.jokers.mult_mod; extras.mult = true end
                if effects.jokers.chip_mod then hand_chips = mod_chips(hand_chips + effects.jokers.chip_mod);extras.hand_chips = true end
                if effects.jokers.Xmult_mod then mult = mult*effects.jokers.Xmult_mod; extras.mult = true  end
                update_hand_text({delay = 0}, {chips = extras.hand_chips and hand_chips, mult = extras.mult and mult})
            ...
            for _, v in ipairs(G.jokers.cards) do
                ...
                if effect then
                    local extras = {mult = false, hand_chips = false}
                    if effect.mult_mod then mult = mult + effect.mult_mod; extras.mult = true end
                    if effect.chip_mod then hand_chips = mod_chips(hand_chips + effect.chip_mod);extras.hand_chips = true end
                    if effect.Xmult_mod then mult = mult*effect.Xmult_mod; extras.mult = true  end
            ...
            if edition_effects.jokers then
                if edition_effects.jokers.x_mult_mod then
                    mult = mult*edition_effects.jokers.x_mult_mod
                    update_hand_text({delay = 0}, {mult = mult})
                    card_eval_status_text(_card, 'jokers', nil, percent, nil, {
            ...
            G.E_MANAGER:enqueue_event({
                func = function()
                    if cards_destroyed[i].ability.name == 'Glass Card' then 
                        ...
                        cards_destroyed[i]:start_fx_mask()
                    end
                  return true
                end
              })
...
G.Fs.evaluate_round = function()
    ...
    G.E_MANAGER:enqueue_event({
        func = function()
            ease_background_color_blind(G.g_states.round_eval, '')
            return true
        end
    })
    ...
    if G.GAME.dollars >= 5 and not G.GAME.modifiers.no_interest then
        ...
        if not G.GAME.seeded and not G.GAME.challenge then
            if G.GAME.interest_amount*math.min(math.floor(G.GAME.dollars/5), G.GAME.interest_cap/5) == G.GAME.interest_amount*G.GAME.interest_cap/5 then 
                G.g_profile[G.SET.profile].career_stats.c_round_interest_cap_streak = G.g_profile[G.SET.profile].career_stats.c_round_interest_cap_streak + 1
            else
                G.g_profile[G.SET.profile].career_stats.c_round_interest_cap_streak = 0
            end
        end
...
G.Fs.tutorial_controller = function()
    if G.F.skip_tut then
        G.SET.tutorial_complete = true
        G.SET.tutorial_progress = nil
        return
    end
    G.SET.tutorial_progress = G.SET.tutorial_progress or 
    {
    ...
    if not G.SET.pause and (not G.SET.tutorial_complete) then
        if G.g_state == G.g_states.blind_select and G.blind_select and not G.SET.tutorial_progress.completed_parts['small_blind'] then
            G.SET.tutorial_progress.section = 'small_blind'
            G.Fs.tutorial_part('small_blind')
            G.SET.tutorial_progress.completed_parts['small_blind']  = true
            G:save_progress()
        end
        if G.g_state == G.g_states.blind_select and G.blind_select and not G.SET.tutorial_progress.completed_parts['big_blind'] and G.GAME.round > 0 then
            G.SET.tutorial_progress.section = 'big_blind'
            G.Fs.tutorial_part('big_blind')
            G.SET.tutorial_progress.completed_parts['big_blind']  = true
            G.SET.tutorial_progress.forced_tags = nil
            G:save_progress()
        end
        if G.g_state == G.g_states.select_hand and not G.SET.tutorial_progress.completed_parts['second_hand'] and G.SET.tutorial_progress.hold_parts['big_blind'] then
            G.SET.tutorial_progress.section = 'second_hand'
            G.Fs.tutorial_part('second_hand')
            G.SET.tutorial_progress.completed_parts['second_hand']  = true
            G:save_progress()
        end
        if G.SET.tutorial_progress.hold_parts['second_hand'] then
            G.SET.tutorial_complete = true
        end
        if not G.SET.tutorial_progress.completed_parts['first_hand_section'] then 
            if G.g_state == G.g_states.select_hand and not G.SET.tutorial_progress.completed_parts['first_hand'] then
                G.SET.tutorial_progress.section = 'first_hand'
                G.Fs.tutorial_part('first_hand')
                G.SET.tutorial_progress.completed_parts['first_hand']  = true
                G:save_progress()
            end
            if G.g_state == G.g_states.select_hand and not G.SET.tutorial_progress.completed_parts['first_hand_2'] and G.SET.tutorial_progress.hold_parts['first_hand']  then
                G.Fs.tutorial_part('first_hand_2')
                G.SET.tutorial_progress.completed_parts['first_hand_2']  = true
                G:save_progress()
            end
            if G.g_state == G.g_states.select_hand and not G.SET.tutorial_progress.completed_parts['first_hand_3'] and G.SET.tutorial_progress.hold_parts['first_hand_2']  then
                G.Fs.tutorial_part('first_hand_3')
                G.SET.tutorial_progress.completed_parts['first_hand_3']  = true
                G:save_progress()
            end
            if G.g_state == G.g_states.select_hand and not G.SET.tutorial_progress.completed_parts['first_hand_4'] and G.SET.tutorial_progress.hold_parts['first_hand_3']  then
                G.Fs.tutorial_part('first_hand_4')
                G.SET.tutorial_progress.completed_parts['first_hand_4']  = true
                G.SET.tutorial_progress.completed_parts['first_hand_section']  = true
                G:save_progress()
            end
        end
         if G.g_state == G.g_states.shop and G.shop and G.shop.VT.y < 5 and not G.SET.tutorial_progress.completed_parts['shop_1'] then
            G.SET.tutorial_progress.section = 'shop'
            G.Fs.tutorial_part('shop_1')
            G.SET.tutorial_progress.completed_parts['shop_1']  = true
            G.SET.tutorial_progress.forced_voucher = nil
            G:save_progress()
        end
    end
end

G.Fs.tutorial_part = function(_part)
    local step = 1
    G.SET.pause = true
    if _part == 'small_blind' then 
        step = tutorial_info({
            text_key = 'sb_1',
            attach = {major = G._room_r, type = 'cm', offset = {x = 0, y = 0}},
            step = step,
        })
        step = tutorial_info({
            text_key = 'sb_2',
        ...
        step = tutorial_info({
            text_key = 'sb_3',
            highlight = {
                G.blind_select.UIRoot.children[1].children[1].config.object:get_UI_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[1].config.object:get_UI_by_ID('blind_desc'),
            },
        ...
        step = tutorial_info({
            ...
            snap_to = function() 
                if G.blind_select and G.blind_select.UIRoot and G.blind_select.UIRoot.children[1] and G.blind_select.UIRoot.children[1].children[1] and G.blind_select.UIRoot.children[1].children[1].config.object then 
                    return G.blind_select.UIRoot.children[1].children[1].config.object:get_UI_by_ID('select_blind_button') end
                end,
            attach = {major =  G.blind_select.UIRoot.children[1].children[1], type = 'tr', offset = {x = 2, y = 4}},
            step = step,
            no_button = true,
    ...
    elseif _part == 'big_blind' then 
        step = tutorial_info({
            text_key = 'bb_1',
            highlight = {
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UI_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UI_by_ID('blind_desc'),
            },
            hard_set = true,
        ...
        step = tutorial_info({
            ...
            highlight = {
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UI_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[2].config.object:get_UI_by_ID('tag_desc'),
            },
            attach = {major =  G.HUD, type = 'cm', offset = {x = 0, y = -2}},
            step = step,
        })
        step = tutorial_info({
            text_key = 'bb_3',
            highlight = {
                G.blind_select.UIRoot.children[1].children[3].config.object:get_UI_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[3].config.object:get_UI_by_ID('blind_desc'),
            },
            attach = {major =  G.HUD, type = 'cm', offset = {x = 0, y = -2}},
            step = step,
        })
        step = tutorial_info({
            ...
            highlight = {
                G.blind_select.UIRoot.children[1].children[3].config.object:get_UI_by_ID('blind_name'),
                G.blind_select.UIRoot.children[1].children[3].config.object:get_UI_by_ID('blind_desc'),
                G.blind_select.UIRoot.children[1].children[3].config.object:get_UI_by_ID('blind_extras'),
                G.HUD:get_UI_by_ID('hud_ante')
            },
            attach = {major =  G.HUD, type = 'cm', offset = {x = 0, y = -2}},
            step = step,
        })
        step = tutorial_info({
            ...
            snap_to = function() 
                ...
                G.blind_select.UIRoot.children[1].children[2].config.object then 
                    return G.blind_select.UIRoot.children[1].children[2].config.object:get_UI_by_ID('select_blind_button') end
                end,
            button_listen = 'select_blind'
        })
    elseif _part == 'first_hand' then
        ...
        step = tutorial_info({
            text_key = 'fh_2',
            highlight = {
                G.HUD:get_UI_by_ID('hand_text_area')
            },
            attach = {major = G._room_r, type = 'cm', offset = {x = 0, y = 0}},
            step = step,
        })
        step = tutorial_info({
            ...
            attach = {major = G._room_r, type = 'cm', offset = {x = 0, y = 0}},
            highlight = {
                G.HUD:get_UI_by_ID('run_info_button')
            },
            no_button = true,
            button_listen = 'run_info',
            snap_to = function() return G.HUD:get_UI_by_ID('run_info_button') end,
            step = step,
        })
    elseif _part == 'first_hand_2' then
        ...
        step = tutorial_info({
            ...
            highlight = {
                G.hand,
                G.buttons:get_UI_by_ID('play_button'),
                G.HUD:get_UI_by_ID('run_info_button')
            },
            attach = {major = G.hand, type = 'cl', offset = {x = -1.5, y = 0}},
            no_button = true,
    ...
    elseif _part == 'first_hand_3' then
        step = tutorial_info({
            ...
            highlight = {
                G.hand,
                G.buttons:get_UI_by_ID('discard_button'),
                G.HUD:get_UI_by_ID('run_info_button')
            },
            attach = {major = G.hand, type = 'cl', offset = {x = -1.5, y = 0}},
            no_button = true,
    ...
    elseif _part == 'first_hand_4' then
        step = tutorial_info({
            ...
            text_key = 'fh_7',
            highlight = {
                G.HUD:get_UI_by_ID('hud_hands').parent,
            },
            attach = {major = G._room_r, type = 'cm', offset = {x = 0, y = 0}},
            step = step,
        })
        step = tutorial_info({
            ...
            highlight = {
                G.HUD:get_UI_by_ID('hud_hands').parent,
                G.HUD:get_UI_by_ID('row_dollars_chips'),
                G.HUD_blind
            },
            attach = {major = G._room_r, type = 'cm', offset = {x = 0, y = 0}},
            step = step,
    ...
    elseif _part == 'shop_1' then
        step = tutorial_info({
            ...
            highlight = {
                G.SHOP_SIGN,
                G.HUD:get_UI_by_ID('dollar_text_UI').parent.parent.parent
            },
        ...
        step = tutorial_info({
            ...
            highlight = function() return {
                G.SHOP_SIGN,
                G.HUD:get_UI_by_ID('dollar_text_UI').parent.parent.parent,
                G.consumables.cards[#G.consumables.cards],
            } end,
            attach = {major = G.shop, type = 'tm', offset = {x = 0, y = 6}},
            step = step,
        })
        ...
        step = tutorial_info({
            text_key = 's_12',
            highlight = function() return {
                G.shop:get_UI_by_ID('next_round_button'),
            } end,
            snap_to = function() if G.shop then return G.shop:get_UI_by_ID('next_round_button') end end,
            no_button = true,
    ...
    G.E_MANAGER:enqueue_event({
        ...
        timer = 'real_s',
        func = function()
            if (G.UI.overlay_tut.step == step and
            not G.UI.overlay_tut.step_complete) or G.UI.overlay_tut.skip_steps then
                if G.UI.overlay_tut.Jimbo then G.UI.overlay_tut.Jimbo:remove() end
                if G.UI.overlay_tut.content then G.UI.overlay_tut.content:remove() end
                G.UI.overlay_tut:remove()
                G.UI.overlay_tut = nil
                G.SET.tutorial_progress.hold_parts[_part]=true
                return true
            end
            return G.UI.overlay_tut.step > step or G.UI.overlay_tut.skip_steps
        end
    }) 
    G.SET.pause = false
end

function win_game() G.Fs.victory(G) end
function end_round()
    G.E_MANAGER:enqueue_event({
      trigger = 'after',
      delay = 0.2,
      func = function()
        ...
        G.RESET_JIGGLES = true
            ...
            if game_over then
                G.g_state = G.g_states.game_over
                if not G.GAME.won and not G.GAME.seeded and not G.GAME.challenge then 
                    G.g_profile[G.SET.profile].high_scores.current_streak.amt = 0
                end
                G:save_settings()
                G.f_handler.force = true
                G.state_comp = false
            else
                ...
                if G.GAME.current_round.hands_played == 1 then 
                    ...
                    if not G.GAME.seeded and not G.GAME.challenge  then
                        G.g_profile[G.SET.profile].career_stats.c_single_hand_round_streak = 0
                        G:save_settings()
                    end
                end
                ...
                if game_won and not G.GAME.win_notified then
                    ...
                    G.E_MANAGER:enqueue_event({
                        ...
                        blockable = false,
                        func = (function()
                            if G.g_state == G.g_states.round_eval then 
                                win_game()
                                G.GAME.won = true
                                return true
                            end
                ...
                G.Fs.draw_from_hand_to_discard()
                if G.GAME.blind:get_type() == 'Boss' then
                ...
                G.Fs.draw_from_discard_to_deck()
                G.E_MANAGER:enqueue_event({
                    trigger = 'after',
                    delay = 0.3,
                    func = function()
                        G.g_state = G.g_states.round_eval
                        G.state_comp = false
...
function new_round()
    ...
    G.E_MANAGER:enqueue_event({
      ...
      func = function()
            ...
            for k, v in pairs(G.run_card_id) do
